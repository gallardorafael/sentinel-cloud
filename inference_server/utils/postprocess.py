...
def draw_bounding_boxes(image, bboxes, color=(0, 255, 0), thickness=2):
    """Draws bounding boxes on a cv2 image given their coordinates in (x1, y1, x2, y2) format.

    Args:
        image (np.ndarray): The cv2 image on which to draw the bounding boxes.
        bboxes (list): A list of bounding boxes, where each bounding box is a NumPy array of shape (4,)
                      containing the coordinates in (x1, y1, x2, y2) format.
        color (tuple, optional): The color of the bounding boxes. Defaults to green (0, 255, 0).
    ...
    Raises:
        ValueError: If any bounding box has invalid coordinates (e.g., x1 > x2 or y1 > y2).
    """
    ...
    for bbox in bboxes:
        ...
        # Ensure color values are within 0-255 range for OpenCV
